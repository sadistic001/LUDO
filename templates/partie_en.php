...
if(! valider("connect","SESSION"))
	header("Location:index.php?view=inscription_en");
...
function changer(refthis) {
	...
	if(play%2==0 && pos_rouge==-1 && refthis.id == "des1"){
		...
		if (nb==6) {
			...
			reftour.innerHTML="turn of <?= $jouer ?>";		
		nb=spindice(refthis)+1;
		if (nb==6) {
			pos_jaune=0;
		...
		reftour.innerHTML="turn of <?= $joueur1 ?>";		
		deplacer_rouge(pos_rouge);
		nb=spindice(refthis)+1;
		pos_rouge+=nb;
		testfin();		
...
function testfin(){
	if(pos_jaune>=61){
		pos_jaune=61;	
		document.location.href="controleur.php?action=win&gagnant="+<?php echo $datajoueur["J_id"];?>+"&loser="+<?php echo $_SESSION["idUser"];?>;
	}	

	else if(pos_rouge>=61){
		pos_rouge=61;
		document.location.href="controleur.php?action=win&gagnant="+<?php echo $_SESSION["idUser"];?>+"&loser="+<?php echo $datajoueur["J_id"];?>;
	}
}
...
?>
<h3 id="tour">turn of <?php echo $_SESSION["pseudo"] ?></h3>
<?php echo "<a href=\"controleur.php?action=leave&loser=$_SESSION[idUser]&gagnant=$datajoueur[J_id]&joueur1=$joueur1\"><img class=\"croix\" id=\"croix1\" src=\"ressources/croix.png\"></a>";
 echo "<a href=\"controleur.php?action=leave&loser=$datajoueur[J_id]&gagnant=$_SESSION[idUser]&joueur1=$joueur1\"><img class=\"croix\" id=\"croix2\" src=\"ressources/croix.png\"></a>";
?>
